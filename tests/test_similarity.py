...
def test_index_never_exceeds_one_with_many_overlapping_sources():
    """Three near-identical sources, one copied passage.

    The index must count the passage once while the report still names every
    source it could have come from, because that list is what lets a reader
    tell deliberate copying from a shared textbook definition.
    """
    sources = {
        "Source A": SOURCE_A,
    ...
    # every plausible origin is listed, not only the one that claimed the span
    assert len(sim.by_source) >= 2
    # shares are the uniquely attributed words, so they sum to the index
    assert abs(sum(s.share for s in sim.by_source) - sim.index) < 1e-9
    assert sum(s.unique_words for s in sim.by_source) == sim.matched_words
    # the duplicates account for the same words without inflating the index
    assert all(s.words >= s.unique_words for s in sim.by_source)
    assert sum(s.words for s in sim.by_source) > sim.matched_words
    assert any(s.unique_words == 0 for s in sim.by_source)
