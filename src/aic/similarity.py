...
def merge_matches(matches: Iterable[Match]) -> list[Match]:
    """Keep the strongest match for each region of the submission.

    Matches are not summed. If a sentence appears in five sources it is one
    copied sentence, and the report lists all five sources while the index
    counts its words once.
    """
    ordered = sorted(matches, key=lambda m: (-m.n_words, -m.similarity))
    kept: list[Match] = []
    claimed: list[tuple[int, int]] = []
    for m in ordered:
        q0, q1 = m.query_tokens
        overlap = sum(
            max(0, min(q1, c1) - max(q0, c0) + 1) for c0, c1 in claimed
        )
        if overlap > 0.5 * m.n_words:
            continue
        kept.append(m)
        claimed.append((q0, q1))
    kept.sort(key=lambda m: m.query_tokens[0])
    return kept
